...
В Mikrotik `.rsc` все адреса добавляются в общий `address-list` (`to_vpn`) с
общим `comment` (`rbgeoip`) — так же, как и остальные списки. Имя из конфига
влияет только на имена выходных файлов. Чтобы добавить новый ASN — допишите его
в `asn-lists.json`.
